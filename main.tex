...
It is also the largest component in the graph, and can be usually seen in many real world networks~\footnote{For example, in the case of the internet network, the existence of a giant component is extremely important, as it guarantees that nodes can communicate with each other}.
...
One important property where the random graph model does not mimic the Real world network properties is it Is with the distribution of degree.
Of the degree is the property of the node in a graph, and it describes how many connections the node has to be other notes in the graph.
...
One of the these heavy tail distributions is the power law distribution, which is scale-free, as was and was shown by~\cite{Albert:2002p4071} to to lead to small world phenomenon~\cite{Watts:1998db}.
...
\section{The ramping parameter and credit concentration risk} % (fold)
...
The ramping parameter monitors how the components of a graph (which represents the considered portfolio) decay as edges are removed increasingly according to their strength.
It computes a curve that parameterizes the dependence of the effective exposure on the ramping parameter.
