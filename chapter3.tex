...
\begin{enumerate}
	\item the space of correlation matrices $\mathcal{C}_{n\times n} = \left( C, \mathcal{F},  P_C \right)$,
...
\noindent where,
\begin{itemize}
    \item[] $C \in \R^{n \times n}$, such that $c_{ij} = c_{ji}$, $c_{ii} = 0$, and all elements $c_{ij}$ are i.i.d. random variables
	\item[] $\mathcal{F}$ is the Borel $\sigma-$algebra on $C$
	\item[] $A \in \{0,1\}^{n \times n}$
	\item[] $P_A = \begin{cases} 1 & \text{with probability } p,\\0 & \text{with probability } 1-p\end{cases}$, where $p \in [0,1]$ is given
